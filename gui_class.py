...
class Gui1:
    ...
    def signin_and_signup_buttons(self):
        ...
        button1 = tk.Button(button_frame, text="Sign up", command=lambda: self.open_signin_signup_window("Sign up"), width=15, height=3, font=("Arial", 12), bg="light sky blue")
        button1.grid(row=0, column=0, padx=10)

        # Create the second button and place it in the button frame
        button2 = tk.Button(button_frame, text="Sign in", command=lambda: self.open_signin_signup_window("Sign in"), width=15, height=3, font=("Arial", 12), bg="hot pink")
    ...
    def open_signin_signup_window(self, comm):
        ...
        if comm == "Sign in":
            login_button = tk.Button(self.root, text="Sign in", bg="SpringGreen2", command=self.on_login_button_click)
            login_button.pack(pady=10)
        elif comm == "Sign up":
            signup_button = tk.Button(self.root, text="Sign up", bg="SpringGreen2", command=self.on_signup_button_click)
            signup_button.pack(pady=10)

        signup_button = tk.Button(self.root, text="back", bg="firebrick1", command=self.signin_and_signup_buttons)
    ...
    def chat_window(self):
        ...
        # Send button
        self.send_button = tk.Button(self.root, text="Send", width=10, bg="gold", command=self.send_message)
        ...
        # help guide button
        self.help_button = tk.Button(self.root, text="help", width=15, bg="tomato", command=self.help_guide_func)
        self.help_button.grid(row=2, column=1, padx=10, pady=5)

        # get usernames lst button
        self.users_lst_button = tk.Button(self.root, text="users in chat", width=15, bg="light blue", command=lambda: self.send_funcs_msg_func("get all usernames"))
        self.users_lst_button.grid(row=2, column=0, padx=10, pady=5)

        # send private message
        self.users_lst_button = tk.Button(self.root, text="send private msg", width=15, bg="light blue", command=self.new_window_for_priv_msg)
    ...
    def admin_buttons(self):
        ...
        censored_word_button = tk.Button(self.root, text="edit censored words", width=15, bg="seaGreen2", command=self.new_window_for_censored_words)
        censored_word_button.grid(row=5, column=0, padx=10, pady=5)

        # mute button
        mute_button = tk.Button(self.root, text="mute users", width=15, bg="seaGreen2", command=lambda: self.new_window_for_mute_unmute("mute"))
        mute_button.grid(row=4, column=1, padx=10, pady=5)

        # unmute button
        unmute_button = tk.Button(self.root, text="unmute users", width=15, bg="seaGreen2", command=lambda: self.new_window_for_mute_unmute("unmute"))
        unmute_button.grid(row=4, column=0, padx=10, pady=5)

        # make admin button"
        make_admin_button = tk.Button(self.root, text="make admin", width=15, bg="seaGreen2", command=lambda: self.new_root_label_window("make admin"))
        make_admin_button.grid(row=3, column=1, padx=10, pady=5)

        # kick button"
        make_admin_button = tk.Button(self.root, text="kick a user", width=15, bg="seaGreen2", command=lambda: self.new_root_label_window("kick"))
    ...
    def new_window_for_priv_msg(self):
        ...
        submit_button = tk.Button(root_lil_window, text="send message", bg="gold", command=lambda: self.send_priv_msg_func(username_entry.get(), message_entry.get()))
    ...
    def new_window_for_mute_unmute(self, comm):
        ...
        specific_button = tk.Button(root_lil_window, text="specific user", bg="lightPink1", command=lambda: self.label_window(comm, root_lil_window))
        specific_button.pack(pady=10)

        specific_comm = f"{comm} everyone"
        everyone_button = tk.Button(root_lil_window, text="everyone", bg="CadetBlue1", command=lambda: self.send_funcs_msg_func(specific_comm))
    ...
    def new_window_for_censored_words(self):
        ...
        btn_ok = tk.Button(root, text="add word", bg="lightPink1", command=lambda: self.label_window("add censored word", root))
        btn_ok.pack(pady=10)

        btn_cancel = tk.Button(root, text="remove word", bg="CadetBlue1", command=lambda: self.label_window("remove censored word", root))
    ...
    def label_window(self, comm, root):
        ...
        submit_button = tk.Button(root, text="submit", bg="springGreen2", command=lambda: self.submit(comm, root))
        submit_button.pack(pady=5)

    # sends the data that was entered to the server (with a func from the client)
    def submit(self, comm, root):
